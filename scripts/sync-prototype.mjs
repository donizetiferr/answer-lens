...
export async function expectedPrototype(root = ROOT) {
  ...
  const html = once(source.get('index.html'), '  <main id="main"', `  ${banner}\n  <main id="main"`);
  ...
  sw = once(sw, `\${PREFIX}${appVersion}`, `\${PREFIX}${appVersion}-catalog-r3`);
  output.set('sw.js', sw);
  const resources = ['styles.css', 'icon.svg', 'sw.js', 'src/app.js', 'src/core.js', 'src/storage.js', 'src/demo.js', 'src/output.js', 'resources.json'];
  ...
  Object.assign(metadata.colecao, {
    ...
    fonteDaVersao: { arquivo: 'resources.json', campo: 'appVersion' }
  });
  ...
  const common = {
    ...
    cenarios: [
      { id: 'read-decide-copy-repeat', titulo: 'Ler, decidir, copiar e repetir', descricao: 'Demonstração sintética, veredito obrigatório, notas opcionais, nomes após revelar, cópia ou alternativa em texto e próximo par conservando só a pergunta.' },
      { id: 'isolated-save', titulo: 'Salvar sem acessar o aplicativo real', descricao: 'Chaves v2, legado v1, lock e cache próprios do preview. Nenhum dado real é carregado automaticamente.' }
    ]
  };
  ...
  output.set('resources.json', json({
    ...
    adaptations: [
      ...
      'app.js, core.js, output.js, demo.js, CSS and icon bytes otherwise match the root app. No additional runtime dependency.'
    ],
